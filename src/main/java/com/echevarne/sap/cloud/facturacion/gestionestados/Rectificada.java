...
@Component(Rectificada.CODIGO)
@Slf4j
public class Rectificada extends FacturacionEstado {

	public static final String CODIGO = "R";
	...
	public boolean doTransicion(SolicitudIndividual solicitudIndividual, MasDataEstado estadoOrigen, boolean manual) {
		...
		boolean todasRectificadas = solicitudIndividual.getItems().stream().allMatch(x -> estadosSet.contains(x.obtieneTrazabilidad().getUltimoEstado()));
		//Solo pasaremos a este estado si todas sus pruebas están rectificadas
		log.info(" Petición: " + solicitudIndividual.getPurchaseOrderByCustomer() + " todasRectificadas: " + todasRectificadas);
		return todasRectificadas;
	}
